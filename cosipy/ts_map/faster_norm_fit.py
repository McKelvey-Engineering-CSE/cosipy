...
class FasterNormFit:
    ...
    obtaining :math:`d` counts where :math:`\lambda` is expected on
    ...
    .. note::
        ...
        error in the estimation of the background is small compared to
    ...
    Args:
        ...
        conv_frac_tol (float): Convergence stopping condition,
            expressed as the ratio between the size of the last step
            and the current norm value.
        zero_ts_tol (float): If zero_ts_tol < TS < 0, then TS is set
            to 0 without failed flag status (analytically, TS < 0
            should never happen)
    ...
    def ts(data, bkg, unit_excess, ue_sum, norm):
        ...
        # NB: on Intel processors, np.log1p, like most transcendentals,
        # uses a much slower implementation in Numba than in regular
        # Numpy. Install the icc_rt package and make sure Numba has
        # SVML support enabled (run numba -s to check) to fix.
    ...
    def solve(self, data, bkg, unit_excess, ue_sum):
        ...
        Return:
            (float, float, bool): ts, norm, and failure status (False = success)
        ...
        if dts0 < 0:
            logger.warning("Using underfluctuation code")

            # Underfluctuation

            # A negative norm is not well-defined since we use a
            # Poisson likelihood. When negative norm is allowed,
            ...
            return (ts, norm, False)

        # Newton's method
        norm, conv = self.newton_loop(data, bkg, unit_excess, ue_sum,
