...
def read_burst_params(params_path):
    ...
    with open(params_path, "r") as params:
        locline = params.readline().split(" ")
        specline = params.readline().split(" ")
...
def read_orientation_history(orientation_path):
    """
    Read the history of detector orientations

    Parameters
    ----------
    orientation_path : path
      path to file containing complete orieintation history of detector
    ...
    # to the GRB
    ori_full = np.load(orientation_path)
...
def get_local_bkg_model(bkg_model_path,
    ...
    bkg_model = Histogram.open(bkg_model_path).astype(np.float32)
