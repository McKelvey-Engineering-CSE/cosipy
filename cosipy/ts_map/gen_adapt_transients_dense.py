...
def write_sample_src(df, output_file, length, fluence):
    ...
    times = np.random.normal(loc=length/2, scale=(length/2)/2, size=2*len(df))
...
def write_sample_bg(df, output_file, length, prior):
    ...
    # make up times according to uniform light cugve
...
n_bursts_per_src_dir = int(sys.argv[1])   # number of bursts per source direction
output_dir  = Path(sys.argv[2])  # where to write output bursts
...
for src_dir in src_dirs:
    ...
    for i in range(n_bursts_per_src_dir):
        ...
        src_fluence = np.random.rand() * (5.0 - 0.5) + 0.5
        src_mean = 44600 * src_fluence

        # number of events expected from each bg component
        bg_time  = np.random.rand() * (5.0 - 0.25) + 0.25
