...
from .fast_norm_fit_interp import FastNormFit as fnf_interp
from .fast_norm_fit_numba import FastNormFit as fnf_numba

import logging
logger = logging.getLogger(__name__)

# Switch between versions of fast_norm_fit
fast_norm_fit = fnf_interp
...
class FastTSMap():
    ...
    def get_ei_cds_array(hypothesis_coord, energy_channel, response_path, spectrum, cds_frame, orientation = None):
        ...
        if cds_frame == "local":
            ...
            with FullDetectorResponse.open(response_path) as response:
                ...
                dwell_time_map = orientation.get_dwell_map(target_coord = hypothesis_coord,
                                                           base = response)
    ...
    def fast_ts_fit(hypothesis_coord,
        ...
        # start the fit
        fit = fast_norm_fit(max_iter=1000)
        result = fit.solve(data_cds_array, bkg_model_cds_array, ei_cds_array, np.sum(ei_cds_array))

        return [pix, result[0], result[1], result[2], result[3], result[4]]
    ...
    def fit(self, nside, energy_channel, spectrum, ts_scheme = "RING",
            start_method = "fork", cpu_cores = None, ts_nside = None,
            pixel_idx = [None], fast_norm_opt="orig"):
        ...
        cpu_cores : int, optional
            The number of cpu cores you wish to use for the parallel computation (the default is `None`, which implies using all the available number of cores to perform the parallel computation).
        ...
        global fast_norm_fit
        if fast_norm_opt == "numba":
            fast_norm_fit = fnf_numba
        else:
            fast_norm_fit = fnf_interp

        hypothesis_coords = self.get_hypothesis_coords(nside = nside)
        ...
        if (data_cds_array[bkg_model_cds_array ==0] !=0).sum() != 0:
            #raise ValueError("You have data!=0 but bkg=0, check your
            # inputs!")  let's try to set the data bin to zero when
            # the corresponding bkg bin isn't zero.  Need further
            # investigate, why bkg = 0 but data!=0 happens? ==> it's
            # more like an issue related to simulated data instead of
            # code This first happened in GRB fitting, but got fixed
            # somehow <== I now understand it's caused by using
            # different PsiChi binning in the same fit But it also
            # happened to Crab while the PsiChi binning are both
            # galactic for Crab and the Albedo, why???? ?_?
        ...
        if cpu_cores is None or cpu_cores > total_cores:
            # if you don't specify the number of cpu cores to use or
            # the specified number of cpu cores is the same as the
            # total number of cores you have it will use the
            # total_cores number of cores to run the parallel
            # computation.
            cores = total_cores
        else:
            cores = cpu_cores
        ...
        results = pool.starmap(FastTSMap.fast_ts_fit,
                               FastTSMap.zip_comp(hypothesis_coords, [energy_channel], [data_cds_array], [bkg_model_cds_array],
    ...
    @staticmethod
    def _plot_ts(ts_array, skycoord = None, containment = None,
                 save_plot = False, save_dir = "", save_name = "ts_map.png", dpi = 300):
        ...
        if containment is not None:
            ...
            hp.mollview(m_ts[:], max = max_ts, min = max_ts-critical,
                        title = f"Containment {percentage}%", coord = "G", hold = True)
        else:
